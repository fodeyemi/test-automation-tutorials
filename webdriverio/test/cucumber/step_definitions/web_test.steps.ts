...
When(
  /^Bob searches for "([^"]*)" using the lucky button$/,
...
Then(
  /^Bob should be navigated to the expected website$/,
...
When(
  /^Bob navigates to "([^"]*)" page via "([^"]*)"$/,
  async (typeScriptPageLink: string, docLink: string) => {
...
Then(
  /^Bob should be able to click on all navigation menu links$/,
  async (table: TableDefinition) => {
    const hashes = table.hashes();
...
Then(
  /^Bob should be able to scroll into "([^"]*)" section$/,
  async (followUsLink: string) => {
    await webPage.assertScrollToSectionLink(followUsLink);
  }
);
Then(
  /^the "([^"]*)" link should be displayed$/,
  async (moreAboutUsLink: string) => {
